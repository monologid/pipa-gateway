...
PipaGateway.prototype = {
    ...
    open: function () {
        var self = this;
        this.readConfig(this.config,function(err,config){
            if(err){
                ...
                _.forOwn(config.routes, function(value, key) {
                ...
                item.services.forEach(function(service){
                    workers.push(function(data,cb){
                        ...
                        if(!cb){
                            ...
                            self._get_words_between_curlies(url).forEach(function(item){
                        ...
                        if(!cb){
                            ...
                            if(service.body){
                                _.forOwn( service.body, function(value, key) {
                                    var curlies = self._get_words_between_curlies(value);
                                    if(curlies!=null){
    ...
    ___get__request: function (uri,headers,params,qs,cb){
        ...
        uri= this._build_new_url(uri,params);
        
        var options = {
            method:'GET',
            uri: uri,
            qs:qs,
            headers:this._delete_default_headers(headers),
            json: true 
        };
        request(options,cb);
    },
    ...
    _build_new_url : function(uri,params) {
        let self = this;
        _.forOwn(params, function(value_param, key_param) {
               uri =  uri.replace(`:${key_param}`,value_param);
        });
        let dolar_curlies = self._get_words_between_dolar_curlies(uri);
        dolar_curlies.forEach(function(item){
            let domainName = item.split(".")[1];
            uri = uri.replace("${"+item+"}",self.domain[domainName]);
        });
       return uri;
    },

    _delete_default_headers : function(headers) {
        delete headers.host;
    ...
    _get_words_between_curlies : function(str) {
    ...
    _get_words_between_dolar_curlies : function(str) {
      var results = [], re = /\${([^}]+)}/g, text;
