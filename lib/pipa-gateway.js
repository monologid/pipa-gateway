...
PipaGateway.prototype.open = function() {
  ...
  this.readConfig(this.configPath, function(err, configData) {
    ...
    let domainFromEnv = helper.constructDomainObjectFromEnvironmentVariables()
    if (domainFromEnv) self.domain = domainFromEnv;

    if (configData.prefix) self.prefix = configData.prefix;

    let prefixFromEnv = helper.constructPrefixFromEnvironmentVariables()
    if (prefixFromEnv) self.prefixes = prefixFromEnv;
...
PipaGateway.prototype.__init__routes = function(methodPath, config) {
  ...
  if (self.prefix) endpoint = helper.generatePrefix(self.prefixes, self.prefix) + endpoint
