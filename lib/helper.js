...
module.exports = {
  ...
  *
  *   Params
  *   @prefixes   string    URL string
  *   @prefix     string    Object parameters
  */

  generatePrefix: function(prefixes, prefix) {
    var self = this;

    var dollarCurlies = self.getWordsBetweenDollarCurlies(prefix);
    dollarCurlies.forEach(function(item) {
      var prefixName = item.split(".")[1];
      prefix = prefix.replace("${" + item + "}", prefixes[prefixName]);
    });
  ...
  */
  ...
  constructPrefixFromEnvironmentVariables: function() {
    const prefix = 'PIPA_GATEWAY_PREFIX_';
